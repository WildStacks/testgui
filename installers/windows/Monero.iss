; WildStacks GUI Wallet Installer for Windows
; Copyright (c) 2021, The wildstacks Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\wildstacks-wallet-gui.exe")

[Setup]
AppName=WildStacks GUI Wallet
...
DefaultDirName={commonpf}\WildStacks GUI Wallet
DefaultGroupName=WildStacks GUI Wallet
UninstallDisplayIcon={app}\wildstacks-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The WildStacks Developer Community
AppPublisherURL=https://wildstacks.net
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the WildStacks files can be copied to a place where they have at least
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to WildStacks.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy
Source: "LICENSE"; DestDir: "{app}"; Flags: ignoreversion

; WildStacks GUI wallet exe and guide
Source: "bin\wildstacks-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\wildstacks-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; WildStacks CLI wallet
Source: "bin\extras\wildstacks-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\wildstacks-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; WildStacks wallet RPC interface implementation
Source: "bin\extras\wildstacks-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; WildStacks daemon
Source: "bin\wildstacksd.exe"; DestDir: "{app}"; Flags: ignoreversion

; WildStacks daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "wildstacks-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; WildStacks blockchain utilities
Source: "bin\extras\wildstacks-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\wildstacks-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\wildstacks-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\wildstacks-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\wildstacks-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\wildstacks-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\wildstacks-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\wildstacks-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\wildstacks-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\wildstacks-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\wildstacks-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\wildstacks-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitwildstacks');
  s := 'The default folder to store the WildStacks blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\wildstacks-project\wildstacks-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" WildStacks default location
...
begin
  Result := BlockChainDir('') + '\bitwildstacks.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "wildstacks-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the WildStacks daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\wildstacksd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\wildstacks-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "WildStacks GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\wildstacks-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\wildstacks-gui-wallet-guide.pdf"; IconFilename: "{app}\wildstacks-wallet-gui.exe"
...
Name: "{group}\Utilities\WildStacks Daemon"; Filename: "{app}\wildstacksd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\wildstacks-wallet-cli.exe"; WorkingDir: "{userdocs}\WildStacks\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\WildStacks\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\wildstacks-wallet-gui\wildstacks-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\wildstacks-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM wildstacksd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\wildstacks-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\wildstacks-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\wildstacks-project\wildstacks-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\wildstacks-project\wildstacks-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "wildstacks:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "wildstacks://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "wildstacks"; ValueType: "string"; ValueData: "URL:WildStacks Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "wildstacks"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "wildstacks\DefaultIcon"; ValueType: "string"; ValueData: "{app}\wildstacks-wallet-gui.exe,0"
Root: HKCR; Subkey: "wildstacks\shell\open\command"; ValueType: "string"; ValueData: """{app}\wildstacks-wallet-gui.exe"" ""%1"""
